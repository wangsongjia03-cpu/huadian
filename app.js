(() => {
  ...
  const FONT_FAMILY = '"LXGW WenKai Lite", "STKaiti", "KaiTi", serif'
  const NOTEBOOK = {
    fillColor: 'rgba(255, 253, 247, 0.9)',
    lineColor: 'rgba(113, 157, 193, 0.34)',
    borderColor: 'rgba(124, 157, 184, 0.22)',
    lineGap: 86,
    radius: 24
  }
  ...
  function drawNotebookPaper(ctx) {
    ctx.save()
    roundedRectPath(ctx, TEXT.x, TEXT.y, TEXT.width, TEXT.height, NOTEBOOK.radius)
    ctx.fillStyle = NOTEBOOK.fillColor
    ctx.fill()
    ...
    ctx.beginPath()
    for (let y = TEXT.y + NOTEBOOK.lineGap; y < TEXT.y + TEXT.height; y += NOTEBOOK.lineGap) {
      ctx.moveTo(TEXT.x, y)
      ctx.lineTo(TEXT.x + TEXT.width, y)
    }
    ctx.strokeStyle = NOTEBOOK.lineColor
    ctx.lineWidth = 2
    ctx.stroke()
    ctx.restore()
  }
  ...
  async function generateCard() {
    ...
    try {
      ...
      if (document.fonts?.load) {
        await document.fonts.load(`${TEXT.fontSize}px "LXGW WenKai Lite"`, message)
      }
