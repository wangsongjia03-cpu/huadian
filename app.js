(() => {
  ...
  const PHOTO = { x: 190, y: 450, width: 858, height: 660 }
  ...
  function updateMessage() {
    ...
    elements.characterCount.textContent = `${Array.from(message).length} / 34`
  }
  ...
  function drawWritingLines(ctx) {
    ctx.save()
    ctx.beginPath()
    for (let index = 1; index <= TEXT.maxLines; index += 1) {
      const lineY = TEXT.y + index * TEXT.lineHeight - 1
      ctx.moveTo(TEXT.x, lineY)
      ctx.lineTo(TEXT.x + TEXT.width, lineY)
    }
  ...
  async function generateCard() {
    ...
    try {
      ...
      if (document.fonts?.load) {
        await document.fonts.load(`${TEXT.fontSize}px "Yozai"`, message)
      }
