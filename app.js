(() => {
  ...
  function drawNotebookPaper(ctx) {
    ctx.save()
    ctx.beginPath()
    for (let y = NOTEBOOK.y + NOTEBOOK.lineGap; y < NOTEBOOK.y + NOTEBOOK.height; y += NOTEBOOK.lineGap) {
      ctx.moveTo(NOTEBOOK.x, y)
      ctx.lineTo(NOTEBOOK.x + NOTEBOOK.width, y)
    }
